...
const handleType = async (type, gridElement) => {
  ...
  // Protección: el botón puede no existir en el DOM (ej. si solo usas 'ground')
  ...
  try {
    ...
    renderTypeGrid(list);
    const label = document.getElementById(type + "-btn")?.textContent || type;
    setStatus(`Listo: ${list.length} Pokémon de tipo ${label}.`, "ok");
...
const handleSearch = async (event) => {
  ...
  try {
    ...
    renderSingle(null);
    setStatus("De seguro pusiste un nombre mal o exediste los 1025 pokemon existentes, por favor intenta de nuevo", "error");
  }
};

searchForm?.addEventListener("submit", handleSearch);

// Registramos únicamente el botón para 'Tierra' (ground)
document.getElementById("ground-btn")?.addEventListener("click", () => handleType("ground", typeGrid));
